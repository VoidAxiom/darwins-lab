...
(the `--json` typo from the source prompt is corrected here):
...
**Future milestone:** Codex app-server bridge (`turn/steer`) for true
active-turn steering. Not dropped — deferred.
...
runtime/visual feedback loop using its own tools: the headless inspection
harness (`npm run inspect` → `inspect/*.png` + `report.json`) and the
**chrome-devtools MCP** (live DOM, screenshots, console/network errors).
Codex may still be asked to *write* regression tests/fixtures/seeded
scenarios as bounded packets, but running and judging runtime behaviour stays
with Claude. Human-reported runtime bugs become regression checks where
practical.
...
feedback, not interruption. Codex review replies go **in-thread, tagged
`@codex`** (never a top-level comment), and threads are resolved only after
Codex re-evaluates. See `CLAUDE.md` for the review-gate mechanics.
...
explicitly authorized. `.env` keys exist but are **not** authorized for
routine spend (they power the app's optional metered narrator only).
...
- Codex is **subscription-covered, not metered** — not a money gate. Only the
  app's `.env` narrator keys are metered/gated.
- **Unverified here (no codex/network):** the `model` id and the exact
  `-o result.json` ⊕ `--json`→stdout interaction. `scripts/codex-run.sh`
  is defensive (always keeps `events.jsonl`/`stdout.md`; derives the result
  from `result.json` if present, else the last JSON event) and `CODEX_MODEL`
  is a single env var. Run one smoke task and inspect the packet before
  trusting these.
- `auto_review` (mid-run self-unblock) ≠ the PR merge gate (`@codex review`
  + conversation resolution + `verify` check). Both coexist.
